var keysBackgound = $('#keys').not('.no-animate').css('background-position').split(" ");
...
function EasyPeasyParallax() {
	...
	if(windowWidth > 980){
		...
		var o = 1-(scrollPos/175),
		 text = $('.logop').not('.no-animate'),
			scroll_down = $('.scroll-down').not('.no-animate');
		if (o <= 0 ){
			o = 0.001;
			scroll_down.hide();
		}
		...
		var y = Number(keysBackgound[1].replace(/[^0-9-.,]/g, ''));

		if(scrollPos != 0 && (scrollPos + y) < -150){
			$('#keys').not('.no-animate').css({
				'background-position' : (-170 - scrollPos * 0.5) +'px ' + (y +(scrollPos*1.5))+"px"
			});
		}
...
$('#numberUser').keyup(function(){

	if ( $('#numberUser').val() > 10) {
		$('#numberUser').val($('#numberUser').attr('max'));
	}
});

function someResize(){
	EasyPeasyParallax();
	var	windowWidth = $(window).width()-42,
...
$(window).scroll(function() {
	...
	$('.quest_block').not('.no-animate').each(function(){
